...
/* --- Короткие описания задач для панели «взглянуть» -------------- */

const taskDescriptions: Record<string, string> = {
  "LH-2481":
    "Подключаем шлюз ЦБ: авторизация по сертификату, сверка остатков раз в сутки и повтор неуспешных операций.",
  "LH-2484": "Возврат средств полной суммой и частями, с записью операции в журнал и уведомлением клиента.",
  "LH-2486": "Отчёт по сверке в формате CSV с разделителем «;» и листом согласования до 14 июня.",
  "LH-2490": "Проверяем обработку вебхуков: подпись HMAC, повтор при 504 и порядок событий.",
  "LH-2500": "Ключи хранятся в защищённом контуре, доступ выдаётся по заявке и отзывается автоматически.",
};

export function taskDescription(task: { code: string; title: string; assignee: string }) {
  return (
    taskDescriptions[task.code] ??
    `Работа по направлению «${task.title}». Подробности, чек-лист и обсуждение — на экране задачи.`
  );
}
